...
echo "OPS235 Lab 8 Check Script" > $logfile
echo | tee -a $logfile
...
check "ssh ops235@centos3 test -s /var/lib/dhcpd/dhcpd.leases " "This program did not detect the NON-EMPTY file called \"/var/lib/dhcpd/dhcpd.leases\" file in your centos3 VM. Please make corrections, and re-run this checking shell script." | tee -a $logfile

# Check that centos3 can ping centos host (IPADDR: 192.168.235.42)
echo "Checking that centos3 VM can ping centos1 host (IPADDR: \"192.168.235.42\"): " | tee -a $logfile
check "ssh ops235@centos3 ping -c1 192.168.235.42 > /dev/null 2> /dev/null" "This program did not detect that there was an ip address set for any network interface card (i.e. eth0) for the value: \"192.168.235.42\". Please make corrections, and re-run this checking shell script." | tee -a $logfile
